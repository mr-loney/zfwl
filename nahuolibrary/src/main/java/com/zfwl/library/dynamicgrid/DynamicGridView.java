package com.zfwl.library.dynamicgrid;
...
public class DynamicGridView extends GridView {
    ...
    private boolean mIsLastDraggable = true;
    private boolean mScrollable = true;
    private boolean mKeepEditModeAfterActionUp = false;
    ...
    public boolean isLastDraggable() {
        return mIsLastDraggable;
    }

    public void setLastDraggable(boolean isLastDraggable) {
        mIsLastDraggable = isLastDraggable;
    }
    ...
    private void startWobbleAnimation() {
        for (int i = 0; i < getChildCount(); i++) {
            if(!mIsLastDraggable && i == getChildCount() -1){
                return;
            }
            View v = getChildAt(i);
    ...
    private BitmapDrawable getAndAddHoverView(View v) {
        ...
        int extraW = (int) (0.1*w);
        int extraH = (int) (0.1*h);
        mHoverCellOriginalBounds = new Rect(left - extraW, top - extraH, left + w + + extraW, top + h + extraH);
    ...
    private void updateNeighborViewsForId(long itemId) {
        ...
        int lastVisiblePosition = mIsLastDraggable ? getLastVisiblePosition() : getLastVisiblePosition() - 1;
        for (int pos = getFirstVisiblePosition(); pos <= lastVisiblePosition; pos++) {
            if (draggedPos != pos) {
...
//            }
    ...
    public boolean isKeepOnEditModeOnActionUp() {
        return mKeepEditModeAfterActionUp;
    }

    public void setKeepOnEditModeOnActionUp(boolean keepOnEditModeOnActionUp) {
        mKeepEditModeAfterActionUp = keepOnEditModeOnActionUp;
    }
