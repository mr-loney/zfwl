package com.zfwl.library.controls.pulltorefresh;
...
import com.zfwl.library.controls.pulltorefresh.internal.CircleLoading;
...
public abstract class PullToRefreshBase<T extends View> extends LinearLayout {
    ...
    private CircleLoading headerLayout;
    private CircleLoading footerLayout;
    ...
    public final void setIsAutoRefresh(boolean isAutoRefresh) {
    ...
    /**
     * @description MODE_PULL_DOWN_TO_REFRESH = 0x1; MODE_PULL_UP_TO_REFRESH = 0x2; MODE_BOTH = 0x3;
     * @created 2015年5月27日 下午2:09:17
     * @author JorsonWong
     */
        ...
        switch (event.getAction()) {
            ...
            case MotionEvent.ACTION_UP: {
                if (isBeingDragged) {
                    ...
                    if (state == RELEASE_TO_REFRESH && currentMode == MODE_PULL_DOWN_TO_REFRESH
                            && null != onRefreshListener) {
                        setRefreshingInternal(true);
                        onRefreshListener.onRefresh();
                    } else if (state == RELEASE_TO_REFRESH && currentMode == MODE_PULL_UP_TO_REFRESH
                            && null != onRefreshListener) {
                        setRefreshingInternal(true);
    ...
    protected final CircleLoading getFooterLayout() {
        return footerLayout;
    }

    protected final CircleLoading getHeaderLayout() {
        return headerLayout;
    }
    ...
    private void init(Context context, AttributeSet attrs) {
        ...
        String pullUpLabel = context.getString(R.string.pull_to_refresh_footer_pull_label);
        String pullUpRefreshingLabel = context.getString(R.string.pull_to_refresh_footer_refreshing_label);
        String pullUpReleaseLabel = context.getString(R.string.pull_to_refresh_footer_release_label);

        // Add Loading Views
        if (mode == MODE_PULL_DOWN_TO_REFRESH || mode == MODE_BOTH) {
            headerLayout = new CircleLoading(context, MODE_PULL_DOWN_TO_REFRESH, releaseLabel, pullLabel,
                    refreshingLabel);
        ...
        if (mode == MODE_PULL_UP_TO_REFRESH || mode == MODE_BOTH) {
            footerLayout = new CircleLoading(context, MODE_PULL_UP_TO_REFRESH, pullUpReleaseLabel, pullUpLabel,
                    pullUpRefreshingLabel);
    ...
    private boolean pullEvent() {
        ...
        Log.i(getClass().getName(), "newHeight:" + newHeight + " headerHeight:" + headerHeight + " percent:" + (-((Math.abs(newHeight) / (float) headerHeight))));
        switch (currentMode) {
            case MODE_PULL_UP_TO_REFRESH:
                footerLayout.setProgress(-((Math.abs(newHeight) / (float) headerHeight)));
                break;
            case MODE_PULL_DOWN_TO_REFRESH:
                headerLayout.setProgress(-((Math.abs(newHeight) / (float) headerHeight)));
                break;
        }
    ...
    public void setCanLoadMore(boolean canLoadMore) {
        if (canLoadMore) {
            mode = currentMode = MODE_BOTH;
        } else {
            mode = currentMode = MODE_PULL_DOWN_TO_REFRESH;
        }
